...
import {
  TooltipEmojis,
  TooltipMessages,
} from "../constants/tooltipMessages.ts";
...
const useTooltip = () => {
  ...
  const getTooltipMessageByLetterStatus = useCallback(
    (status: LetterStatus): TOOLTIP_MESSAGE => {
      switch (status) {
  ...
  const triggerTooltip = useCallback(
    (type: TOOLTIP_MESSAGE, row: number, cell: number) => {
