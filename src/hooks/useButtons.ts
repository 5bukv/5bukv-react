...
function shouldUpdateStatus(
  currentStatus: LetterStatus | null,
  newStatus: LetterStatus,
): boolean {
  if (newStatus === LetterStatus.CORRECT) return true;
  if (
    newStatus === LetterStatus.WRONG_PLACE &&
    currentStatus !== LetterStatus.CORRECT
  )
    return true;
  return (
    newStatus === LetterStatus.NOT_IN_WORD &&
    currentStatus === LetterStatus.DEFAULT
  );
}
...
function updateButton(button: KeyboardButton, newStatus: LetterStatus) {
  if (
    ...
    shouldUpdateStatus(button.status, newStatus)
  ) {
...
const useButtons = () => {
  ...
  const updateButtonStatus = useCallback(
    (symbol: string, newStatus: LetterStatus) => {
  ...
  const resetButtons = useCallback(() => {
    setButtons(
      initialButtons.map((row) => row.map((button) => ({ ...button }))),
    );
  }, []);
